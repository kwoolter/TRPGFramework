...
from .RPGGameStats import *
from .RPGItems import ItemFactory
...
class Character(RPGObject):
    ...
    # Add all of the derived stats required for the game
    def add_derived_stats(self):

        self.add_stat(CoreStat("XP", "Core Stats",0))
        self.add_stat(CoreStat("Gold", "Core Stats",0))
        self.add_stat(CoreStat("Level","Abilities",1))

        self.add_stat(MaxLoad(self))
        self.add_stat(Lockpicking(self))
        self.add_stat(HasMoved(self))
        self.add_stat(XPToLevel(self))
        self.add_stat(LevelUP(self))
        self.add_stat(MaxHP(self))
        self.add_stat(HP(self))
        self.add_stat(TotalWeight(self))
        self.add_stat(LoadPct(self))
    ...
    def examine(self):
        ...
        attributes = list(Character.CORE_STAT_NAMES)
        attributes += ["Weight","XP", "Level","XPToLevel", "LevelUp", "HP", "Max HP"]
...
class RPGCSVFactory(object):
    ...
    def load(self):
        ...
        with open(self.file_name, 'r') as rpg_object_file:
            ...
            for row in reader:
                ...
                if rpg_object_name not in self._rpg_objects.keys():
                    new_rpg_object_stats = []
                    self._rpg_objects[rpg_object_name] = new_rpg_object_stats

                # Get the set of stats for this RPG object
                rpg_object_stats = self._rpg_objects[rpg_object_name]
                ...
                for i in range(1,len(header)):
                    ...
                    new_stat = CoreStat(field, self.stat_category, int(row.get(field)))
    ...
    def get_rpg_object_names(self):
        return self._rpg_objects.keys()

    # Return the list of stats for the specified object name
    def get_stats_by_name(self, object_name : str):
        if object_name in self._rpg_objects.keys():
            return deepcopy(self._rpg_objects[object_name])
        else:
            return None
    ...
    def get_matching_objects(self, stat : BaseStat):
        ...
        for rpg_object_name in self._rpg_objects.keys():
            # Loop through it's stats...
            for rpg_stat in self._rpg_objects[rpg_object_name]:
            ...
            for row in reader:
                ...
                for stat_name in other_header_names:
                    ...
                    if is_numeric(stat_value) is not None:
                        ...
                        new_stat_list.append(CoreStat(stat_name,"NPC", stat_value))
