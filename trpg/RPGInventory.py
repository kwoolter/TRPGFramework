...
class Inventory(object):
    ...
    def __init__(self, character : Character, items : ItemFactory):
    ...
    def add_item(self, item : Item):
        ...
        if self._character.type != Character.TYPE_VENDOR and item.weight > self.max_load:
            raise Exception("%s is too big for %s to pick up." % (item.name, self._character.name))
        # Else If...
        # ...you are not a vendor...
        # ...and you can't carry the additional weight
        elif self._character.type != Character.TYPE_VENDOR and item.weight + self.total_weight > self.max_load:
